...
function iniciarMotor(cardId, ativoId, nomeAtivo) {
    ...
    m.ws.on('message', (data) => {
        ...
        if (res.ohlc) {
            ...
            // 1. MENSAGEM DE ALERTA ATUALIZADA
            if (s >= 50 && s <= 55 && !m.op.ativa && !m.alertado) {
                ...
                if (pattern && pattern.dir === tendM5 && emaOk) {
                    ...
                    enviarTelegram(`⚠️ *ALERTA BRAIN PRO*\n\n📊 Ativo: ${m.nome}\n🎯 Padrão: ${pattern.name}\n📈 Filtro M5+EMA: ✅\n🕓 Possível horário de entrada: ${hEntrada}`);
                    m.alertado = true;
                }
            }

            // 2. MENSAGEM DE ENTRADA ATUALIZADA
            if (s === 0 && !m.op.ativa) {
                m.alertado = false;
        ...
        if (m.op.ativa) {
            ...
            if (m.op.t <= 0) {
                ...
                if (ganhou) {
                    ...
                    // 3. MENSAGEM DE GREEN ATUALIZADA
                    ...
                    m.op.t = 60; 
                    m.op.pre = parseFloat(m.preco);
                    const h = obterHorarios();
                    
                    // 4. MENSAGEM DE GALE ATUALIZADA
                    enviarTelegram(`⚠️ *Gale ${m.op.g}*\n👉Clique agora!\n📊 Ativo: ${m.nome}\n🎯 Padrão: ${m.op.est}\n📈 Direção: ${m.op.dir}\n💰 Valor: R$ ${m.op.val.toFixed(2)}\n⏰ Inicio: ${h.inicio}\n🏁 Fim: ${h.fim}`);
                } else {
                    stats.loss++;
                    // MENSAGEM DE RED NO MESMO FORMATO DO GREEN
...
app.post('/config-financeira', (req, res) => {
    ...
    if (banca !== undefined) { fin.bancaInicial = Number(banca); fin.bancaAtual = Number(banca); }
...
app.listen(PORT, () => console.log(`BRAIN PRO ATUALIZADO RODANDO NA PORTA ${PORT}`));
